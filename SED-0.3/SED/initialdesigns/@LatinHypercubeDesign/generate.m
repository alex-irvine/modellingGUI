...
if s.prefabInternet
	...
	try
		...
		if any(accumarray(initialsamples(:)+1, 1) ~= inDim)
			msg = sprintf( 'Prefab LHD design for %i samples in %i dimensions could not be downloaded automatically: invalid file detected', s.points, inDim);
			s.logger.warning(msg);
		...
		% all ok
			...
			try
				...
				s.logger.warning(['Failed to save prefab LHD design: ' ME.message ]);
			end
		...
		s.logger.warning(msg);
		s.logger.warning(['Prefab error message: ' ME.message ]);
		
	end
end

if s.statisticsToolbox
	initialsamples = lhsdesign(s.points, inDim, 'criterion', 'maximin', 'iterations', 1000000, 'smooth', 'off');
...
plot(initialsamples(:,1), initialsamples(:,2), 'or')
evaluatedSamples = [];
